...
class PatientScheduleFaceTransformer extends TransformerAbstract
{
    ...
    public function transform(PatientScheduleFace $patientSchedule)
    {
        ...
        $numberPending = resolve('PatientScheduleFace')->getModel()->where('patient_id', $patient[0]->id)->where('status', 'Pending')->get()->count();
        $numberApproved = resolve('PatientScheduleFace')->getModel()->where('patient_id', $patient[0]->id)->where('status', 'Done')->get()->count();
